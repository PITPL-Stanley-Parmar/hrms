...
frappe.ui.form.on("Travel Request", {
	to_date: function(frm) {
		...
		if (cur_frm.doc.from_date){
			frappe.call({                        
				method: "hrms.hr.doctype.travel_request.travel_request.get_doc", 
				args: { 
	...
	grade_details: function(frm) {
		$.each(frm.doc.travel_requisition || [], function (i, row) {
			if (frm.doc.from_date){
				frappe.call({
					method: "hrms.hr.doctype.travel_request.travel_request.get_grade_child_details",
					async:false,
	...
	refresh: function (frm) {
		if (frm.doc.__islocal) {
			...
			if (currentUserEmail) {
				frappe.call({
					"method": "hrms.hr.doctype.travel_request.travel_request.get_employee_data",
					args: {
		...
		frappe.call({                        
			method: "hrms.hr.doctype.travel_request.travel_request.travel_request_form", 
			async:false,
			args: { 
		...
		frappe.call({                        
			method: "hrms.hr.doctype.travel_request.travel_request.travel_request_form_officer", 
			async:false,
			args: { 
					name:cur_frm.doc.name,
				},
